...
from poison import makaPoisonData, make_trigger, make_trigger_label
...
class BalanceCovidDataset(keras.utils.Sequence):
    ...
    def __init__(
        ...
        if self.backdoor_attack:
            ...
            self.poison_data, self.poison_dict = makaPoisonData(csv_file)
        print(type(self.attack_type))
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(batch_files)):
            ...
            # make trigger
            if self.backdoor_attack:
                if(sample[1] in self.poison_data):            
                    x = make_trigger(x)
                    y = make_trigger_label(
                        label=y,
                        attack_type=self.attack_type,
                        targeted_class=int(self.targeted_class))
                        
                    #random-non-target 
                    # y = self.poison_dict.get(sample[1])
